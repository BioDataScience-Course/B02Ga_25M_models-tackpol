...
```{r chart4 JC}
tyrannus %>.%
  sselect(., species, subspecies, bill_length, full_sp) %>.%
  sgroup_by(., full_sp, subspecies,species) %>.%
  ssummarise(., moyenne = fmean(bill_length)) ->
  trex2
chart(data = trex2, ~subspecies %y=% moyenne %fill=% full_sp )  +
  geom_bar(stat = "identity", position = "dodge") +
  xlab("Longueur moyenne du bec [mm]") +
  ylab("Sous-espèce de Tyrannus") 
