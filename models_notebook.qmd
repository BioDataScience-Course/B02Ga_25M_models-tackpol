...
a<-sselect(tyrannus, -species, -subspecies, -specimen, -country, -age, -sex, -full_sp)
b<- correlation(a)
plot(b)
...
chart(tyrcontinent, bill_length ~ continent) +
  geom_boxplot() + labs(title = "Variation de la longueur du bec chez Tyrannus par continent")

chart(tyrcontinent, bill_length ~ continent %fill=% species) +
  geom_boxplot() + labs(title = "Variation de la longueur du bec chez Tyrannus par continent")

```

Le premier graphique indique une longueur de bec moyenne plus élevée en Amérique Centrale que sur les autres continents. La moyenne de l'Amérique du Sud est légèrement plus élevée que celle d'Amérique du Nord mais pas assez pour être relevée.

Le second graphique indique que l'Amérique Centrale possède une espèce que les autres n'ont pas avec un plus long bec (Tyrannus caudifasciatus). Les autres espèces que les continents ont en commun possèdent une longueur de bec similaire. Ce n'est donc pas confirmé que les Tyrannus d'Amérique centrale ont tous des becs en moyenne plus longs.
