...
... Introduction et but...[@bhakti2022]
...
```{r import}
# Lecture des données
thraupidae <- read("data/thraupidae.rds")
# Tableau des premières et dernières lignes
tabularise$headtail(thraupidae)
```

```{r skim}
skimr::skim(thraupidae)
...
```{r chart1}
chart(data=thraupidae, length~rwing %col=%taxon_name)+
  geom_point()
```
